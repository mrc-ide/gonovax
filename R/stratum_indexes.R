...
stratum_index_xpvwrh <- function(n_erlang = 1, n_diag_rec = 1, strategy = NULL) {
  ...
  if (!is.null(strategy)){
    if (!is.null(strategy) & (strategy == "VaH" | strategy ==  "VaHonly")){
      ...
      ret$vaccinatedfrom_vos = c(ret$X[-1], ret$X[-1], ret$P[- ( seq_len(n_erlang * n_diag_rec) %% n_diag_rec == 1)] , ret$W[-1])
      ret$vaccinatedto_vos = c(ret$P1[-1], ret$V1[-1], rep(ret$V1[-1], n_erlang), ret$R[-1])
    } else {
