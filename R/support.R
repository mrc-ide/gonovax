...
extract_flows_trial <- function(y, idx) {
  ...
  cumulative_flows$cum_diag_a_VW_first_diag_hist <-
    t(aggregate(y, "cum_diag_a", stratum = c(idx$V[1], idx$W[1])))
  ...
  cumulative_flows$cum_diag_s_VW_first_diag_hist <-
    t(aggregate(y, "cum_diag_s", stratum = c(idx$V[1], idx$W[1])))
  ...
  N_diag_hist_VW <- 
    t(aggregate(y, "N", stratum = c(idx$V[1], idx$W[1])))
  ...
  N_diag_hist_VW_T <- 
    t(aggregate(y, "T", stratum = c(idx$V[1], idx$W[1])))
  
  # obtain person-years exposed before first diagnosis i.e person-years in treatment
  # don't count towards
  ...
  test <- c(cumulative_flows, flows, cum_N_pyrs)
