...
##' @name never_diag_hist_id
##' @title For post-processing of the XVW stochastic trial model, generates
##'  indices of the strata in V in which individuals have not yet been diagnosed
##'  including erlang > 1
##' @param vec a vector of indices for 'V' generated by
##' stratum_index_xvw_trial()
...
never_diag_hist_id <- function(vec, n_erlang, n_diag_rec) {
  ...
  #store first index of V
  ret <- vec[1]

  #if n_erlang = 1 , we just take the first diagnosis history stratum
  #otherwise:

  if (n_erlang != 1) {

    #take all the first diagnosis history of each erlang stratum

    for (i in 2:n_erlang) {

      #add n_diag rec, -1 times as many times as there are n_erlang
      ret <- append(ret, max(ret) + n_diag_rec)
    }
...
##' @param y a transformed model run output
##' @param idx list generated by stratum_index_xvw_trial() showing which
