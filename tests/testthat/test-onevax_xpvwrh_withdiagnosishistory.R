...
test_that("run_onevax_xpvwrh works correctly", {
  ...
  ## Should all work fine n_diag_rec = 1!
  ...
  tt <- seq(0, 5)
  gp <- gono_params(1:2)
  y1 <- run_onevax_xpvwrh(tt, gp, vea = 0, dur_v = 1e3, n_erlang = 1, n_diag_rec = n_diag_rec)[[1]]
  ...
  y2 <- run_onevax_xpvwrh(tt, gp, vea = 0, dur_v = 1e10, vbe = 1, n_diag_rec = n_diag_rec)
  ...
  expect_equal(sum(y2[[1]]$N[, , -c(1:n_diag_rec, (2*n_diag_rec+1):(3*n_diag_rec), (3*n_diag_rec+1):(4*n_diag_rec))]), 0)
  ...
  y2.1 <- run_onevax_xpvwrh(tt, gp, vea = 0, dur_v = 1e3, vbe = 1, hes = 0.5, n_diag_rec = n_diag_rec)
  ...
  expect_equal((rowSums(y2.1[[i]]$cum_vaccinated[, , 5*n_diag_rec+1])), rep(0,
                                                               max(tt) + 1))
}
  ...
  expect_error(y <- run_onevax_xpvwrh(tt, gp, vea = 0, dur_v = 1e3, vbe = 1,
                                     primary_uptake = uptake, n_diag_rec = n_diag_rec))

  # check can restart
  init_params <- lapply(y2, restart_params)
  y3 <- run_onevax_xpvwrh(seq(max(tt), length.out = 2, by = 1),
                         gp, init_params,
                         vea = 0, dur_v = 1e3, vbe = 1, n_diag_rec = n_diag_rec)
  for (i in seq_along(y2)) {
  ...
  y_h <- run_onevax_xpvwrh(tt, gp, vea = 0, dur_v = 1e3, vbe = 1, hes = 0.3, n_diag_rec = n_diag_rec)
  ...
  for (i in seq_along(y_h)) {
    ...
    if (n_diag_rec > 1){
    expect_true(all(y_h[[i]]$N[1, , ((5*n_diag_rec+2):(6*n_diag_rec))] == 0))
    }
    
  }

  # incidence in hesitant population increasing --- Trystan question -> what is this checking?
  for (i in seq_along(y_h)) {
    expect_true(all(y_h[[i]]$cum_incid[-1, , (5*n_diag_rec+1):(6*n_diag_rec)] > 0))
  }

  y_h2 <- run_onevax_xpvwrh(tt, gp, vea = 0, dur_v = 1e3, vbe = 0, hes = 0.3, n_diag_rec = n_diag_rec)
  ...
  # H and X stratum sizes remain constant in time
    ...
    expect_equal(y_h2[[i]]$N[1, , ], y_h2[[i]]$N[6, , ])
      ...
      temp1 = c(rowSums(y_h2[[i]]$N[1, ,1:n_diag_rec]), rowSums(y_h2[[i]]$N[1,, 5*n_diag_rec + (1:n_diag_rec)]))
      temp2 = c(rowSums(y_h2[[i]]$N[6, ,1:n_diag_rec]), rowSums(y_h2[[i]]$N[6,, 5*n_diag_rec + (1:n_diag_rec)]))
    
     # temp1 = c(y_h2[[i]]$N[1,,1] + y_h2[[i]]$N[1,,2], y_h2[[i]]$N[1,,11] + y_h2[[i]]$N[1,,12])
     #  temp2 = c(y_h2[[i]]$N[6,,1] + y_h2[[i]]$N[6,,2], y_h2[[i]]$N[6,,11] + y_h2[[i]]$N[6,,12])
  ...
  y_h3 <- run_onevax_xpvwrh(tt, gp, vea = 0, dur_v = 1e3, hes = 0.5, n_diag_rec = n_diag_rec)
  ...
  for (jj in 1:n_diag_rec){
    ...
    expect_equal(y_h3[[i]]$cum_incid[, , jj], y_h3[[i]]$cum_incid[, , 5*n_diag_rec+jj])
    
  }  
    

  expect_equal(y_h3[[i]]$cum_incid[, , 1], y_h3[[i]]$cum_incid[, , 5*n_diag_rec+1])
  
  if (n_diag_rec == 2){
    expect_equal(y_h3[[i]]$cum_incid[, , 2], y_h3[[i]]$cum_incid[, , 5*n_diag_rec+2])
...
test_that("The vaccination maps are being generated as expected", {
  ...
  i_eligible <- c(1, 1, 2, 4)
    ...
    i_eligible <- c(1:n_diag_rec, 1:n_diag_rec, (n_diag_rec + (1:n_diag_rec)), 3*n_diag_rec + (1:n_diag_rec))
  }
  ...
  i_p <- c(2, 3, 3, 5)
    ...
    i_p <- c((n_diag_rec + (1:n_diag_rec)), 2*n_diag_rec + (1:n_diag_rec), 2*n_diag_rec + (1:n_diag_rec), 4*n_diag_rec + (1:n_diag_rec))
  }
  ...
  vod_map <- create_vax_map_branching(n_vax = idx$n_vax, p$vod, i_eligible, i_p_temp_2,
                                      idx = idx)
  ...
  expect_true(unique(vod_map[, n_diag_rec+d, n_diag_rec+d] == c(1, 1)))
  expect_true(unique(vod_map[, 2*n_diag_rec+d+1, n_diag_rec+d] == c(-1, -1)))
  expect_true(unique(vod_map[, 3*n_diag_rec+d, 3*n_diag_rec+d] == c(1, 1)))
  expect_true(unique(vod_map[, 4*n_diag_rec+d+1, 3*n_diag_rec+d] == c(-1, -1)))
  } else{
    ...
    expect_true(unique(vod_map[, n_diag_rec+d, n_diag_rec+d] == c(1, 1)))
    expect_true(unique(vod_map[, 2*n_diag_rec+d, n_diag_rec+d] == c(-1, -1)))
    expect_true(unique(vod_map[, 3*n_diag_rec+d, 3*n_diag_rec+d] == c(1, 1)))
    expect_true(unique(vod_map[, 4*n_diag_rec+d, 3*n_diag_rec+d] == c(-1, -1)))
    }
  ...
  if (d < n_diag_rec){  
    expect_equal(sum(vod_map[, -c(d, n_diag_rec+d+1, 2*n_diag_rec+d+1), d]), 0)
    expect_equal(sum(vod_map[, -c(n_diag_rec+d, 2*n_diag_rec+d+1), n_diag_rec+d]), 0)
    expect_equal(sum(vod_map[, -c(3*n_diag_rec+d, 4*n_diag_rec+d+1), 3*n_diag_rec+d]), 0)
    expect_equal(sum(vod_map[, , c(2*n_diag_rec+d+1, 5*n_diag_rec+d+1, 4*n_diag_rec+d)]), 0)
  } else{
    expect_equal(sum(vod_map[, -c(d, n_diag_rec+d, 2*n_diag_rec+d), d]), 0)
    expect_equal(sum(vod_map[, -c(n_diag_rec+d, 2*n_diag_rec+d), n_diag_rec+d]), 0)
    expect_equal(sum(vod_map[, -c(3*n_diag_rec+d, 4*n_diag_rec+d), 3*n_diag_rec+d]), 0)
    expect_equal(sum(vod_map[, , c(2*n_diag_rec+d, 5*n_diag_rec+d, 4*n_diag_rec+d)]), 0)
    }
  }
  ...
  expect_true(unique(vos_map[, 2*n_diag_rec+d, n_diag_rec+d] == c(0, -1)))
  expect_true(unique(vos_map[, 3*n_diag_rec+d, 3*n_diag_rec+d] == c(0, 1)))
  expect_true(unique(vos_map[, 4*n_diag_rec+d, 3*n_diag_rec+d] == c(0, -1)))
  }

  
  for (d in (1:n_diag_rec)){
    expect_equal(sum(vos_map[, -c(d, n_diag_rec+d, 2*n_diag_rec+d), d]), 0)
    expect_equal(sum(vos_map[, -c(n_diag_rec+d, 2*n_diag_rec+d), n_diag_rec+d]), 0)
    expect_equal(sum(vos_map[, -c(3*n_diag_rec+d, 4*n_diag_rec+d), 3*n_diag_rec+d]), 0)
    expect_equal(sum(vos_map[, , c(2*n_diag_rec+d, 5*n_diag_rec+d, 4*n_diag_rec+d)]), 0)
  }
  ...
  u <- create_uptake_map_xpvwrh(vod_map, r1[i], r2[i], r2_p[i],
                                booster_uptake[i], idx, n_diag_rec = n_diag_rec, n_erlang = 1, screening_or_diagnosis = "diagnosis")
  ...
  expect_true(unique(acc_vax[, n_diag_rec+d+1, d] == c(- (r1[i] * (1 - r2[i])),
                                          - (r1[i] * (1 - r2[i])))))
  expect_true(unique(acc_vax[, 2*n_diag_rec+d+1, d] == c(-r1[i] * r2[i], -r1[i] * r2[i])))

  expect_true(unique(acc_vax[, n_diag_rec+d, n_diag_rec+d] == c(r2_p[i], r2_p[i])))
  expect_true(unique(acc_vax[, 2*n_diag_rec+d+1, n_diag_rec+d] == -c(r2_p[i], r2_p[i])))

  expect_true(unique(acc_vax[, 3*n_diag_rec+d, 3*n_diag_rec+d] == c(booster_uptake[i],
                                          booster_uptake[i])))
  expect_true(unique(acc_vax[, 4*n_diag_rec+d+1, 3*n_diag_rec+d] == -c(booster_uptake[i],
                                          booster_uptake[i])))
  } else{
    expect_true(unique(acc_vax[, d, d] == c(r1[i], r1[i])))
    expect_true(unique(acc_vax[, n_diag_rec+d, d] == c(- (r1[i] * (1 - r2[i])),
                                                         - (r1[i] * (1 - r2[i])))))
    expect_true(unique(acc_vax[, 2*n_diag_rec+d, d] == c(-r1[i] * r2[i], -r1[i] * r2[i])))
    
    expect_true(unique(acc_vax[, n_diag_rec+d, n_diag_rec+d] == c(r2_p[i], r2_p[i])))
    expect_true(unique(acc_vax[, 2*n_diag_rec+d, n_diag_rec+d] == -c(r2_p[i], r2_p[i])))
    
    expect_true(unique(acc_vax[, 3*n_diag_rec+d, 3*n_diag_rec+d] == c(booster_uptake[i],
                                                                      booster_uptake[i])))
    expect_true(unique(acc_vax[, 4*n_diag_rec+d, 3*n_diag_rec+d] == -c(booster_uptake[i],
                                                                         booster_uptake[i])))
...
test_that("Test VoD is working correctly", {
  ...
  for (i in seq_along(y3e)) {
    ...
    expect_equal(y3e[[i]]$cum_offered[, , c(1:n_diag_rec, n_diag_rec + 1:n_diag_rec, 3*n_diag_rec + 1:n_diag_rec)],
                 y3e[[i]]$cum_treated[, , c(1:n_diag_rec, n_diag_rec + 1:n_diag_rec, 3*n_diag_rec + 1:n_diag_rec)])
    ...
    expect_equal(rowSums(y3e[[i]]$cum_offered[, , n_diag_rec + 1:n_diag_rec] * r2_p[i]),
                 rowSums(y3e[[i]]$cum_vaccinated[, , n_diag_rec + 1:n_diag_rec]))

    # uptake % of offered booster are vaccinated
    expect_equal(rowSums(y3e[[i]]$cum_offered[, , 3*n_diag_rec + 1:n_diag_rec] * booster_uptake[i]),
                 rowSums(y3e[[i]]$cum_vaccinated[, , 3*n_diag_rec + 1:n_diag_rec]))
    ...
    ## all treated in X, p or W are offered vaccination
    # and no-one else
    expect_equal(sum(y3e[[i]]$cum_offered[, , -c(1:n_diag_rec, n_diag_rec + 1:n_diag_rec, 3*n_diag_rec + 1:n_diag_rec)]), 0)
...
test_that("Test VoA is working correctly", {
  ...
  for (i in seq_along(y4e)) {
    ...
    expect_equal(y4e[[i]]$cum_offered[, , c(1:n_diag_rec, n_diag_rec + 1:n_diag_rec, 3*n_diag_rec+(1:n_diag_rec))], 
                 y4e[[i]]$cum_treated[, , c(1:n_diag_rec, n_diag_rec + 1:n_diag_rec, 3*n_diag_rec+(1:n_diag_rec))] +
                   y4e[[i]]$cum_screened[, , c(1:n_diag_rec, n_diag_rec + 1:n_diag_rec, 3*n_diag_rec + (1:n_diag_rec))])
    # and no-one else
    expect_equal(sum(y4e[[i]]$cum_offered[, , -c(1:n_diag_rec, n_diag_rec+(1:n_diag_rec), 3*n_diag_rec+(1:n_diag_rec))]), 0)
    ...
    # uptake % of 1st dose offered 2nd dose are vaccinated
    expect_equal(rowSums(y4e[[i]]$cum_offered[, , n_diag_rec + 1:n_diag_rec] * r2_p[i]),
                 rowSums(y4e[[i]]$cum_vaccinated[, , n_diag_rec + 1:n_diag_rec]))

    # uptake % of offered booster are vaccinated
    expect_equal(rowSums(y4e[[i]]$cum_offered[, , 3*n_diag_rec + 1:n_diag_rec] * booster_uptake[i]),
                 rowSums(y4e[[i]]$cum_vaccinated[, , 3*n_diag_rec + 1:n_diag_rec]))
    # and no-one else
    expect_equal(sum(y4e[[i]]$cum_vaccinated[, , -c(1:n_diag_rec, n_diag_rec + 1:n_diag_rec, 3*n_diag_rec + 1:n_diag_rec)]), 0)
...
test_that("Test vaccination targeting", {
  ...
  for (i in seq_along(y5e)) {
    ...
    expect_equal(y5e[[i]]$cum_offered[, 1, c(1:n_diag_rec, n_diag_rec + 1:n_diag_rec, 3*n_diag_rec+(1:n_diag_rec))],
                 y5e[[i]]$cum_treated[, 1, c(1:n_diag_rec, n_diag_rec + 1:n_diag_rec, 3*n_diag_rec+(1:n_diag_rec))])
    ## H who are treated or screened in X, P, or W are offered vaccination
    expect_equal(y5e[[i]]$cum_offered[, 2, c(1:n_diag_rec, n_diag_rec + 1:n_diag_rec, 3*n_diag_rec+(1:n_diag_rec))],
                 y5e[[i]]$cum_treated[, 2,  c(1:n_diag_rec, n_diag_rec + 1:n_diag_rec, 3*n_diag_rec+(1:n_diag_rec))] +
                   y5e[[i]]$cum_screened[, 2, c(1:n_diag_rec, n_diag_rec + 1:n_diag_rec, 3*n_diag_rec+(1:n_diag_rec))])
    # and no-one else
...
test_that("Test length of uptake vector must be 1 or length gp", {
  ...
  expect_error(run_onevax_xpvwrh(tt, gp, vea = 1, dur_v = 1e3, strategy = "VbE",
                                r1 = c(0, 0.5, 1),  n_diag_rec = n_diag_rec))
  expect_error(run_onevax_xpvwrh(tt, gp, vea = 1, dur_v = 1e3, strategy = "VbE",
                                r1 = 1, r2 = c(0, 0.5, 1), n_diag_rec = n_diag_rec))
  expect_error(run_onevax_xpvwrh(tt, gp, vea = 1, dur_v = 1e3, strategy = "VbE",
                                 r1 = 1, r2 = 1, r2_p = c(0, 0.5, 1), n_diag_rec = n_diag_rec))
  expect_error(run_onevax_xpvwrh(tt, gp, vea = 1, dur_v = 1e3, strategy = "VbE",
                                 r1 = 1, r2 = 1, booster_uptake = c(0, 0.5, 1), n_diag_rec = n_diag_rec))

  # check length of vea must be 1
  expect_error(run_onevax_xpvwrh(tt, gp, vea = c(0, 1, 1), dur_v = 1e3,
                                strategy = "VbE", r1 = 1, n_diag_rec = n_diag_rec))
...
test_that("Test revaccination is working", {
  ...
  for (i in seq_along(y6e)) {
    ## L who are treated in X, P, or W are offered vaccination
    expect_equal(y6e[[i]]$cum_offered[, 1, c(1:n_diag_rec, n_diag_rec + 1:n_diag_rec, 3*n_diag_rec+(1:n_diag_rec))],
                 y6e[[i]]$cum_treated[, 1, c(1:n_diag_rec, n_diag_rec + 1:n_diag_rec, 3*n_diag_rec+(1:n_diag_rec))])
    ## H who are treated or screened in X, P, or W are offered vaccination
    expect_equal(y6e[[i]]$cum_offered[, 2, c(1:n_diag_rec, n_diag_rec + 1:n_diag_rec, 3*n_diag_rec+(1:n_diag_rec))],
                 y6e[[i]]$cum_treated[, 2, c(1:n_diag_rec, n_diag_rec + 1:n_diag_rec, 3*n_diag_rec+(1:n_diag_rec))] +
                   y6e[[i]]$cum_screened[, 2, c(1:n_diag_rec, n_diag_rec + 1:n_diag_rec, 3*n_diag_rec+(1:n_diag_rec))])
    # and no-one else
    expect_equal(sum(y6e[[i]]$cum_offered[, , -c(1:n_diag_rec, n_diag_rec + 1:n_diag_rec, 3*n_diag_rec+(1:n_diag_rec))]), 0)

    # uptake % of offered are vaccinated
    expect_equal(y6e[[i]]$cum_offered[, , 1:n_diag_rec] * r1[i],
                 y6e[[i]]$cum_vaccinated[, , 1:n_diag_rec])
    expect_equal(y6e[[i]]$cum_offered[, , n_diag_rec + 1:n_diag_rec] * r2_p[i],
                 y6e[[i]]$cum_vaccinated[, , n_diag_rec + 1:n_diag_rec])
    expect_equal(y6e[[i]]$cum_offered[, , 3*n_diag_rec + 1:n_diag_rec] * booster_uptake[i],
                 y6e[[i]]$cum_vaccinated[, , 3*n_diag_rec + 1:n_diag_rec])

    # stratum V empties immediately
    expect_equal(sum(y6e[[i]]$N[, , 2*n_diag_rec + 1:n_diag_rec]), 0, tolerance = 1e-5)

    # efficacy is perfect
    expect_equal(sum(y6e[[i]]$cum_incid[, , c(n_diag_rec + 1:n_diag_rec, 2*n_diag_rec + 1:n_diag_rec, 4*n_diag_rec + 1:n_diag_rec)]), 0)
    expect_true(all(y6e[[i]]$cum_incid[-1, , c(1:n_diag_rec, 3*n_diag_rec + 1:n_diag_rec)] > 0))
  }
  ...
  for (i in seq_along(y7e)) {
    ## L who are treated in X, P, or W are offered vaccination
    expect_equal(y7e[[i]]$cum_offered[, 1, c(1:n_diag_rec, n_diag_rec + 1:n_diag_rec, 3*n_diag_rec+(1:n_diag_rec))],
                 y7e[[i]]$cum_treated[, 1, c(1:n_diag_rec, n_diag_rec + 1:n_diag_rec, 3*n_diag_rec+(1:n_diag_rec))])
    ## H who are treated or screened in X, P or W are offered vaccination
    expect_equal(y7e[[i]]$cum_offered[, 2, c(1:n_diag_rec, n_diag_rec + 1:n_diag_rec, 3*n_diag_rec+(1:n_diag_rec))],
                 y7e[[i]]$cum_treated[, 2,  c(1:n_diag_rec, n_diag_rec + 1:n_diag_rec, 3*n_diag_rec+(1:n_diag_rec))] +
                   y7e[[i]]$cum_screened[, 2,  c(1:n_diag_rec, n_diag_rec + 1:n_diag_rec, 3*n_diag_rec+(1:n_diag_rec))])
    # and no-one else
    expect_equal(sum(y7e[[i]]$cum_offered[, , -c(1:n_diag_rec, n_diag_rec + 1:n_diag_rec, 3*n_diag_rec+(1:n_diag_rec))]), 0)

    # uptake % of offered are vaccinated
    expect_equal(y7e[[i]]$cum_offered[, , 1:n_diag_rec] * r1[i],
                 y7e[[i]]$cum_vaccinated[, , 1:n_diag_rec])
    expect_equal(y7e[[i]]$cum_offered[, , n_diag_rec + 1:n_diag_rec] * r2_p[i],
                 y7e[[i]]$cum_vaccinated[, , n_diag_rec + 1:n_diag_rec])
    expect_equal(y7e[[i]]$cum_offered[, , 3*n_diag_rec + 1:n_diag_rec] * booster_uptake[i],
                 y7e[[i]]$cum_vaccinated[, , 3*n_diag_rec + 1:n_diag_rec])

    # efficacy is perfect in R
    expect_equal(sum(y7e[[i]]$cum_incid[, , 4*n_diag_rec + 1:n_diag_rec]), 0)
    # but not in XPWVH
    expect_true(all(y7e[[i]]$cum_incid[-1, , -c(n_diag_rec + 1:n_diag_rec, 4*n_diag_rec + 1:n_diag_rec)] > 0))
  }
...
test_that("Test restart with hesistancy is working", {
  ...
  y_hesres <- run_onevax_xpvwrh(tt, gp, init_params = i_p, vea = 0, dur_v = 1e3,
                               hes = 0.5, n_diag_rec = n_diag_rec)

  n_erlang <- 2
  y_erlang <- run_onevax_xpvwrh(tt, gp, vea = 0, dur_v = 1e3,
                                n_erlang = n_erlang, n_diag_rec = n_diag_rec)
  i_p <- lapply(y_erlang, restart_hes, n_vax = 3*n_diag_rec + 3*n_diag_rec*n_erlang,
                hes = 0.5, branching = TRUE, n_erlang = n_erlang, n_diag_rec = n_diag_rec)
  y_hesres_erlang <- run_onevax_xpvwrh(tt, gp, init_params = i_p, vea = 0,
                                       dur_v = 1e3,
                                hes = 0.5, n_erlang = n_erlang, n_diag_rec = n_diag_rec)
  ...
  for (i in seq_along(y_hesres)) {
    for (d in 1:n_diag_rec){
    expect_equal(y_hesres[[i]]$U[, , d], y_hesres[[i]]$U[, , 5*n_diag_rec + d])
    expect_equal(y_hesres[[i]]$I[, , d], y_hesres[[i]]$I[, , 5*n_diag_rec + d])
    expect_equal(y_hesres[[i]]$A[, , d], y_hesres[[i]]$A[, , 5*n_diag_rec + d])
    expect_equal(y_hesres[[i]]$S[, , d], y_hesres[[i]]$S[, , 5*n_diag_rec + d])
    expect_equal(y_hesres[[i]]$T[, , d], y_hesres[[i]]$T[, , 5*n_diag_rec + d])
    }
    expect_equal(rowSums(y8[[i]]$U[, , ((n_diag_rec+1):5*n_diag_rec)]), rep(0, length(tt)))
    expect_equal(rowSums(y8[[i]]$I[, , ((n_diag_rec+1):5*n_diag_rec)]), rep(0, length(tt)))
    expect_equal(rowSums(y8[[i]]$A[, , ((n_diag_rec+1):5*n_diag_rec)]), rep(0, length(tt)))
    expect_equal(rowSums(y8[[i]]$S[, , ((n_diag_rec+1):5*n_diag_rec)]), rep(0, length(tt)))
    expect_equal(rowSums(y8[[i]]$T[, , ((n_diag_rec+1):5*n_diag_rec)]), rep(0, length(tt)))
  ...
  for (d in 1:n_diag_rec){  
    ...
    expect_equal(rowSums(y_hesres_erlang[[i]]$U[, , (n_diag_rec+1):(idx$n_vax - n_diag_rec)]),
                 rep(0, length(tt)))
    expect_equal(rowSums(y_hesres_erlang[[i]]$I[, , (n_diag_rec+1):(idx$n_vax - n_diag_rec)]),
                 rep(0, length(tt)))
    expect_equal(rowSums(y_hesres_erlang[[i]]$A[, , (n_diag_rec+1):(idx$n_vax - n_diag_rec)]),
                 rep(0, length(tt)))
    expect_equal(rowSums(y_hesres_erlang[[i]]$S[, , (n_diag_rec+1):(idx$n_vax - n_diag_rec)]),
                 rep(0, length(tt)))
    expect_equal(rowSums(y_hesres_erlang[[i]]$T[, , (n_diag_rec+1):(idx$n_vax - n_diag_rec)]),
                 rep(0, length(tt)))
  ...
  y9 <- run_onevax_xpvwrh(tt, gp, vea = 0, dur_v = 1e3, hes = 0.5, n_diag_rec = n_diag_rec)

  expect_error(lapply(y9, restart_hes, n_vax = 6, hes = 0.5, branching = TRUE, n_erlang = n_erlang, n_diag_rec = n_diag_rec))

  # restart_hes gives error if baseline model run provided contains vaccinated

  y10 <- run_onevax_xpvwrh(tt, gp, vea = 0, dur_v = 1e3, vbe = 1, n_diag_rec = n_diag_rec)
  ...
  y11 <- run_onevax_xpvwrh(tt, gp, r1 = 1, r2 = 0, strategy = "VoD", n_diag_rec = n_diag_rec)

  for (i in seq_along(y11)) {

  expect_equal(sum(y11[[i]]$N[, , -c(1:n_diag_rec, (n_diag_rec + 1):(2*n_diag_rec))]), 0)
  expect_true(all(rowSums(y11[[i]]$N[, , 1:n_diag_rec]) > 0))
  expect_true(all(rowSums(y11[[i]]$N[-1, , (n_diag_rec + 1):(2*n_diag_rec)]) > 0))
  ...
  y12 <- run_onevax_xpvwrh(tt, gp, r1 = 1, r2 = 1, strategy = "VoD", n_diag_rec = n_diag_rec)

  for (i in seq_along(y12)) {

    expect_equal(sum(y12[[i]]$N[, , c((n_diag_rec+1):(2*n_diag_rec), (5*n_diag_rec+1):(6*n_diag_rec))]), 0)
    expect_true(all(rowSums(y12[[i]]$N[, , 1:n_diag_rec]) > 0))
    expect_true(all(rowSums(y12[[i]]$N[-1, , (2*n_diag_rec+1):(5*n_diag_rec)]) > 0))
  ...
  y13 <- run_onevax_xpvwrh(tt, gp, r1 = 1, r2 = 0.5, strategy = "VoD", n_diag_rec = n_diag_rec)

  for (i in seq_along(y13)) {
    for (d in 1:n_diag_rec){
  expect_equal(y13[[i]]$N[, , (n_diag_rec + d)], y13[[i]]$N[, , (2*n_diag_rec + d)])
    }
  }
  ...
  for (i in seq_along(y13)) {
    for ( d in 1:n_diag_rec){
  expect_equal(y13[[i]]$cum_incid[, , (n_diag_rec + d)], y13[[i]]$cum_incid[, , (2*n_diag_rec +d)])
    }
  }
...
test_that("Test vea_p works independently of vex", {
  ...
  for (i in seq_along(y14)) {
    expect_true(all(rowSums(y14[[i]]$N[-1, , (n_diag_rec+1):(2*n_diag_rec)]) > 0))
    expect_true(all(rowSums(y14[[i]]$N[-1, , (2*n_diag_rec + 1):(3*n_diag_rec)]) > 0))
    expect_true(sum(y14[[i]]$cum_incid[-1, , (n_diag_rec+1):(2*n_diag_rec)]) == 0)
    expect_true(all(rowSums(y14[[i]]$cum_incid[-1, , (2*n_diag_rec +1): (3*n_diag_rec)]) > 0))
  }
  ...
  y_vea_veap <- run_onevax_xpvwrh(tt, gp, r1 = 1, r2 = 0.5, vea = 0.5,
                                  vea_p = 0.5, strategy = "VoD", n_diag_rec = n_diag_rec)

  expect_equal(y_vea_only, y_vea_veap)

  # when vea, vea_p and vea_revax = 1 there is no incidence among vaccinated individuals

  y_no_incid <- run_onevax_xpvwrh(tt, gp, r1 = 1, r2 = 0.5, vea = 1, n_diag_rec = n_diag_rec)
  ...
  init_1 <- initial_params_xpvwrh(pars[[1]],
                                  coverage_p = 0.25, coverage_v = 0.25, n_diag_rec = n_diag_rec)

  # expect same number starting in P as in V
  expect_equal(init_1$U0[, n_diag_rec+(1:n_diag_rec)], init_1$U0[, 2*n_diag_rec + (1:n_diag_rec)])

  init_2 <- initial_params_xpvwrh(pars[[1]],
                                 coverage_p = 0.5, coverage_v = 0.25, n_diag_rec = n_diag_rec)

  # expect double starting in P as in V
  expect_equal((init_2$U0[, n_diag_rec+(1:n_diag_rec)]), init_2$U0[, 2*n_diag_rec+(1:n_diag_rec)] * 2)

  # expect error if sum of coverages and hes is greater than 1

  expect_error(initial_params_xpvwrh(pars[[1]], coverage_p = 0.5,
                                     coverage_v = 0.5, hes = 0.5, n_diag_rec = n_diag_rec))

  # expect error if sum of coverages is 100%
  # (if total coverage is 100%, no asymptomatic infection is seeded)

  expect_error(initial_params_xpvwrh(pars[[1]], coverage_p = 0.5,
                                     coverage_v = 0.5, n_diag_rec = n_diag_rec))
  ...
  y15 <- run_onevax_xpvwrh(tt, gp, r1 = 0.5, r2 = 0.5, dur_v = 1e90,
    ...
    sum(y15[[1]]$cum_vaccinated[2, , 1:n_diag_rec])  / sum(y15[[1]]$cum_treated[2, , 1:n_diag_rec])
    
    expect_equal(  sum(y15[[1]]$cum_vaccinated[2, , 1:n_diag_rec])  / sum(y15[[1]]$cum_treated[2, , 1:n_diag_rec]), 0.5)
  ...
  y16 <- run_onevax_xpvwrh(tt, gp, init_params = ip, dur_p = 1e-90, n_diag_rec = n_diag_rec)
  ...
  for (i in seq_along(y16)) {
    ...
    # V W R are empty for all time
      expect_equal(rowSums(y16[[i]]$N[, , (2*n_diag_rec+1):(6*n_diag_rec)]), rep(0, 6))
}
  ...
  ## Trystan note 18 Dec - waning being initialised with numerical error val? (~1e-99?)

  expect_equal(sum(y_p[[i]]$N[1, , n_diag_rec + (1:n_diag_rec)]), 6e+05 * 0.9)
  expect_equal(sum(y_p[[i]]$cum_vaccinated[, , -( (n_diag_rec+1) :(2*n_diag_rec))]), 0)
  expect_true(unique(rowSums(y_p[[i]]$cum_vaccinated[-1, , ( (n_diag_rec+1) :(2*n_diag_rec))]) > 0))
  expect_true(unique(rowSums(y_p[[i]]$N[-1, , (2*n_diag_rec + 1):(3*n_diag_rec)]) > 0))

  # test individuals still wane to W
    ip <- lapply(pars, initial_params_xpvwrh, coverage_v = 0.99999999999999
                 , t = 5, n_diag_rec = n_diag_rec)
    y17 <- run_onevax_xpvwrh(tt, gp, init_params = ip, dur_v = 1e-90, n_diag_rec = n_diag_rec)
    ...
    for (i in seq_along(y17)) {
      expect_equal(sum(y17[[i]]$N[1, , (2*n_diag_rec+1):(3*n_diag_rec)]), 6e+05)
      expect_equal(rowSums(y17[[i]]$N[2:(max(tt) + 1), , (2*n_diag_rec +1):(3*n_diag_rec)]), rep(0, max(tt)))

      for (j in 2:max(tt) + 1) {
      expect_true(sum(y17[[i]]$N[j, , (3*n_diag_rec + 1):(4*n_diag_rec)]) > 0)
      }

                                                 ######## difference in W
                                              ######### isnt quite 1200
    # R, H are empty for all time
      expect_equal(rowSums(y17[[i]]$N[, , (5*n_diag_rec+1):(6*n_diag_rec)]), rep(0, 6))
    }
...
for (j in 1:5){  
  ...
  n_erlang_2 <- initial_params_xpvwrh(pars[[1]], n_erlang = 2, n_diag_rec = n_diag_rec)
  expect_equal(dim(n_erlang_2[[1]]), c(2, 9*n_diag_rec))

  n_erlang_3 <- initial_params_xpvwrh(pars[[1]], n_erlang = 3, n_diag_rec = n_diag_rec)
  expect_equal(dim(n_erlang_3[[1]]), c(2, 12*n_diag_rec))

  n_erlang_10 <- initial_params_xpvwrh(pars[[1]], n_erlang = 10, n_diag_rec = n_diag_rec)
  ...
  y <- run_onevax_xpvwrh(tt, gono_params = gono_params()[1], vea = 0,
                    ...
                    r2 = 0.1, booster_uptake = 1, strategy = "VoD(L)+VoA(H)", n_diag_rec = n_diag_rec)
  ...
  expect_error(run_onevax_xpvwrh(tt, gono_params = gono_params()[1], vea = 0,
                    ...
                    init_params = list(n_erlang_2), n_erlang = 3, n_diag_rec = n_diag_rec))
  ...
  y <- run_onevax_xpvwrh(tt, gono_params = gp, vea = 1, vea_p = 1,
    ...
    # and no one vaccinated is infected
      ...
      expect_true(all(y[[1]]$cum_incid[-1, , (n_diag_rec+1):(n_diag_rec + (n_diag_rec*n_erlang))] == 0))  #P
      expect_true(all(y[[1]]$cum_incid[-1, , (n_diag_rec + (n_diag_rec*n_erlang) + 1):(n_diag_rec + 2*(n_diag_rec*n_erlang))] == 0))  #V
      expect_true(all(y[[1]]$cum_incid[-1, , (2*n_diag_rec + 2*(n_diag_rec*n_erlang) + 1):(2*n_diag_rec + 3*(n_diag_rec*n_erlang))] == 0))  #R
      ...
      expect_true(all(y[[1]]$cum_incid[-1, , (n_diag_rec + 2*(n_diag_rec*n_erlang) + 1):(2*n_diag_rec + 2*(n_diag_rec*n_erlang))] > 0)) #W
  ...
  # to the correct strata
      ...
      #  i_eligible <- c(1, 1, 2, 3, 6)
        ...
        i_eligible <- c(1:n_diag_rec, 1:n_diag_rec, (n_diag_rec+1):(n_diag_rec + n_diag_rec*n_erlang), (n_diag_rec + 2*n_diag_rec*n_erlang + 1):(2*n_diag_rec + 2*n_diag_rec*n_erlang))
      ...
      #  i_p_temp_2[i_p%%n_diag_rec != 0] = i_p[i_p%%n_diag_rec != 0] + n_erlang
        ...
        vod_map <- create_vax_map_branching(n_vax = n_vax, p$vod, i_eligible_temp_2,
                                            i_p_temp_2, idx = idx)
        ...
        for (e in 1:n_erlang){
          for (d in 1:n_diag_rec){
            expect_true(unique(vod_map[, idx$P[(e-1)*n_diag_rec + d], idx$P[(e-1)*n_diag_rec + d]]  == c(1, 1)))
          }
        }
        ...
        for (e in 1:n_erlang){
          for (d in 1:n_diag_rec){
            if (d < n_diag_rec){
              expect_true(unique(vod_map[, idx$V[d + 1], idx$P[(e-1)*n_diag_rec + d]]  == c(-1, -1)))
            }
            else{
              expect_true(unique(vod_map[, idx$V[d], idx$P[(e-1)*n_diag_rec + d]]  == c(-1, -1)))
      ...
      #  expect_true(unique(vod_map[, idx$R[1], idx$W]  == c(-1, -1)))
        ...
        for (d in 1:n_diag_rec){
          if (d < n_diag_rec){
            expect_equal(sum(vod_map[, -c(idx$X[d], idx$P[d+1], idx$V[d+1]), idx$X[d]]), 0)
          }
          else{
            expect_equal(sum(vod_map[, -c(idx$X[d], idx$P[d], idx$V[d]), idx$X[d]]), 0)
          }
        }
        ...
        for (e in 1:n_erlang){
          for (d in 1:n_diag_rec){
            if (d < n_diag_rec){
              expect_equal(sum(abs(vod_map[, -c(idx$P[(e-1)*n_diag_rec + d], idx$V[d+1]), idx$P[(e-1)*n_diag_rec + d]])), 0)
              
            }
            else{
              expect_equal(sum(abs(vod_map[, -c(idx$P[(e-1)*n_diag_rec + d], idx$V[d]), idx$P[(e-1)*n_diag_rec + d]])), 0)
  ...
  #      expect_equal(sum(abs(vod_map[, -c(idx$W, idx$R[1]), idx$W])), 0)
        ...
        for (e in 1:n_erlang){
          for (d in 1:n_diag_rec){
            expect_true(unique(vos_map[, idx$P[(e-1)*n_diag_rec + d], idx$P[(e-1)*n_diag_rec + d]] == c(0, 1)))
            expect_true(unique(vos_map[, idx$V[d], idx$P[(e-1)*n_diag_rec + d]] == c(0, -1)))
          }
        }
       ...
       # }
        ...
        for (d in 1:n_diag_rec){
          expect_equal(sum(vos_map[, -c(idx$X[d], idx$P[d], idx$V[d]), idx$X[d]]), 0)
        ...
        for (e in 1:n_erlang){
          for (d in 1:n_diag_rec){
            expect_equal(sum(abs(vos_map[, -c(idx$P[(e-1)*n_diag_rec+d], idx$V[d]), idx$P[(e-1)*n_diag_rec+d]])), 0)
      ...
      # for vbe, expect:
        ...
        expect_equal(sum(vbe_map[, , 2:(3*n_diag_rec + 3*n_diag_rec*n_erlang)]), 0)
        ...
        for (i in seq_along(r1)) {
          u <- create_uptake_map_xpvwrh(vod_map, r1[i], r2[i], r2_p[i],
                                        booster_uptake[i],
                                        idx = idx, n_diag_rec = n_diag_rec, n_erlang = n_erlang, screening_or_diagnosis = "diagnosis")
          ...
          for (d in 1:n_diag_rec){
            ...
            expect_true(unique(acc_vax[, idx$W[d], idx$W[d]] == c(booster_uptake[i],
                                                            booster_uptake[i])))

            if (d < n_diag_rec){
              expect_true(unique(acc_vax[, idx$P[d+ 1], idx$X[d]] == c(- (r1[i] *
                                                                      (1 - r2[i])),
                                                                 - (r1[i] * (1 - r2[i])))))

              expect_true(unique(acc_vax[, idx$R[d+1], idx$W[d]] == -c(booster_uptake[i],
                                                                  booster_uptake[i])))
            }
            else{
              expect_true(unique(acc_vax[, idx$P[d], idx$X[d]] == c(- (r1[i] *
                                                                                      (1 - r2[i])),
                                                                                 - (r1[i] * (1 - r2[i])))))

              expect_true(unique(acc_vax[, idx$V[d], idx$X[d]] == c(-r1[i] * r2[i],
                                                                 -r1[i] * r2[i])))

              expect_true(unique(acc_vax[, idx$R[d], idx$W[d]] == -c(booster_uptake[i],
                                                                                booster_uptake[i])))
            }
          }
          ...
          for (e in 1:n_erlang){
            for (d in 1:n_diag_rec){
              expect_true(unique(acc_vax[, idx$P[(e-1)*n_diag_rec + d], idx$P[(e-1)*n_diag_rec + d]] == c(r2_p[i],
                                                                                                      r2_p[i])))
              
              if (d < n_diag_rec){
                expect_true(unique(acc_vax[, idx$V[d+1], idx$P[(e-1)*n_diag_rec+d]] == -c(r2_p[i],
                                                                                                   r2_p[i])))
              }
              else{
                expect_true(unique(acc_vax[, idx$V[d], idx$P[(e-1)*n_diag_rec+d]] == -c(r2_p[i],
                                                                                          r2_p[i])))
              }
        ...
        # people wane as expected
          ...
          ip_v <- lapply(pars, initial_params_xpvwrh,
           ...
           y_v <- run_onevax_xpvwrh(tt, gp, init_params = ip_v, n_erlang = 2, n_diag_rec = n_diag_rec)
           ...
           for (e in 1:n_erlang){
             if (e < n_erlang){
               expect_true(sum(rowSums(y_v[[1]]$N[, , idx$V[(e-1)*n_diag_rec+1]])) >
          ...
          # ( + no further vaccination)
           ip_p <- lapply(pars, initial_params_xpvwrh,
                        ...
                        n_erlang = 2, n_diag_rec = n_diag_rec, t = FALSE)
           y_p <- run_onevax_xpvwrh(tt, gp, init_params = ip_p, n_erlang = 2, n_diag_rec = n_diag_rec)
          ...
          idx2 <- stratum_index_xpvwrh(n_erlang)
           ...
           y_long <- run_onevax_xpvwrh(tt, gp, vea = 0, dur_v = 0.1,
            ...
            y_long2 <- run_onevax_xpvwrh(tt, gp, vea = 0, dur_v = 0.1,
                                                  n_erlang = n_erlang,
                                                  r1 = 1, r2 = 1, booster_uptake = 1,
                                                  dur_revax = 1e10,
                                                  strategy = "VoD(L)+VoA(H)")
          ...
          ### Lunch 20 Dec - Trystan note something not working with restart_hes_Diagnosis!

           i_p <- lapply(y_long, restart_hes, n_vax = idx$n_vax, n_erlang = n_erlang, n_diag_rec = n_diag_rec,
                         branching = TRUE)
           tt <- seq(0, 5)
           ...
           y_r <- run_onevax_xpvwrh(tt, gp, init_params = i_p, r1 = 0, r2 = 0,
                                    dur_v = 0.1,
                                    dur_revax = 10, n_erlang = n_erlang, n_diag_rec = n_diag_rec)
      ...
      i_v <- c(idx$P, idx$V, idx$R)
      i_w <- c(idx$P[-(1:n_diag_rec)], idx$X, idx$V[-(1:n_diag_rec)], idx$W, idx$R[-(1:n_diag_rec)], idx$W)
      ...
      for (e in 1:n_erlang){
        for (d in 1:n_diag_rec){
          ...
          expect_true(all(w[idx$P[(e-1)*n_diag_rec + d], idx$P[(e-1)*n_diag_rec + d]] == -n_erlang / dur_p))
          expect_true(all(w[idx$V[(e-1)*n_diag_rec + d], idx$V[(e-1)*n_diag_rec + d]] == -n_erlang / dur_v))
          expect_true(all(w[idx$R[(e-1)*n_diag_rec + d], idx$R[(e-1)*n_diag_rec + d]] == -n_erlang / dur_revax))
          
          
          
          if (e < n_erlang){
            expect_true(all(w[idx$P[(e)*n_diag_rec + d], idx$P[(e-1)*n_diag_rec + d]] == n_erlang / dur_p))
            expect_true(all(w[idx$V[(e)*n_diag_rec + d], idx$V[(e-1)*n_diag_rec + d]] == n_erlang / dur_v))
            expect_true(all(w[idx$R[(e)*n_diag_rec + d], idx$R[(e-1)*n_diag_rec + d]] == n_erlang / dur_revax))
            
            
            
          }
          else{
            expect_true(all(w[idx$X[d], idx$P[(e-1)*n_diag_rec + d]] == n_erlang / dur_p))
            expect_true(all(w[idx$W[d], idx$V[(e-1)*n_diag_rec + d]] == n_erlang / dur_v))
            expect_true(all(w[idx$W[d], idx$R[(e-1)*n_diag_rec + d]] == n_erlang / dur_revax))
