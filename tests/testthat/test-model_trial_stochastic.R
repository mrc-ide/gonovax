...
# the same as test-model_trial_stochastic but for stochastic = TRUE, plus some extra tests
...
test_that("no new infections if lambda is 0", {
  ...
  params$lambda <- 0
  mod <- model_trial_stochastic$new(user = params, unused_user_action = "ignore")
  tt <- seq.int(0, 5)
  y <- mod$run(step = tt)
  ...
  tt <- seq.int(0, 5)
  params$lambda <- 0
  mod2 <- model_trial_stochastic$new(user = params, unused_user_action = "ignore")
  y <- mod2$run(step = tt)
...
test_that("VEa behaves as expected ", {
  ...
  expect_true(sum(y[[1]]$cum_incid[2:6, 2, 3]) > 0)
