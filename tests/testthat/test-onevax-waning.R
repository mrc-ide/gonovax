...
test_that("run_onevax_waning works correctly", {
  ...
  expect_equal(apply(y2e$N, 1, sum), rep(6e5, 4), tolerance = 1e-5)
  ...
  expect_equal(apply(y3e$N, 1, sum), rep(6e5, 4), tolerance = 1e-5)
  ...
  expect_equal(apply(y4e$N, 1, sum), rep(6e5, 4), tolerance = 1e-5)
  ...
  y5e <- run_onevax_waning(1, tt, eff = 1, dur = 1, ve = 1,
                           equilib = TRUE)[[1]]
  expect_equal(apply(y5e$N, 1, sum), rep(6e5, 4), tolerance = 1e-5)
  expect_true(all(apply(y5e$N[-1, , 3], 1, sum) > 0))
  # check that equivalent to waning into U
  z <- run_onevax(1, tt, eff = 1, dur = 1, ve = 1,
                         equilib = TRUE)[[1]]
  expect_equal(z$N[, , 1], y5e$N[, , 1] + y5e$N[, , 3], tolerance = 0.1)
  expect_equal(z$cum_incid[, , 1], y5e$cum_incid[, , 1] + y5e$cum_incid[, , 3],
               tolerance = 0.1)

  # check vaccination targeting
  y6e <- run_onevax_waning(1, tt, eff = 1, dur = 1,
                    vs = c(0, 1), vd = c(0, 1), equilib = TRUE)[[1]]
  expect_equal(y6e$N[, 1, 2], rep(0, 4))
  expect_true(all(y6e$N[-1, 2, 2] > 0))
  expect_true(all(y6e$N[-1, 2, 3] > 0))
})
