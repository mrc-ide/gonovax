...
To calibrate the model using MCMC (as described in [Whittles et al, Lancet ID 2022](https://doi.org/10.1016/S1473-3099(21)00744-1)

Start by defining information about the model parameters being calibrated, such as their names, initial values, minimum, maximum and whether they are discrete or continuous.
```{r}
 library("gonovax")
 parameters_info <- read.csv(system.file("extdata/parameters_info.csv",package='gonovax'))
parameters_info
```

We then define priors for the fitted parameters
```{r}
n_par <- 100
