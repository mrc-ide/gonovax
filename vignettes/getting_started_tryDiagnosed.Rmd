...
tempN2a = numeric(length = 1)
tempN2b = numeric(length = 1)
tempN2c = numeric(length = 1)

tempN2d = numeric(length = 1)



for (i in 1:100){

temp1 = sum(y0[[i]]$cum_diag_a[3,,]) - sum(y0[[i]]$cum_diag_a[2,,])
temp2 = sum(y0[[i]]$cum_diag_s[3,,]) - sum(y0[[i]]$cum_diag_s[2,,]) 

temp3 = sum(y0[[i]]$cum_diag_a[3,,2:n_diag_rec]) - sum(y0[[i]]$cum_diag_a[2,,2:n_diag_rec])
temp4 = sum(y0[[i]]$cum_diag_s[3,,2:n_diag_rec]) - sum(y0[[i]]$cum_diag_s[2,,2:n_diag_rec]) 


temp5 = sum(y0[[i]]$N[3,,])
temp6 = sum(y0[[i]]$N[3,,2:n_diag_rec])


temp11 = sum(y0[[i]]$cum_diag_a[3,,2]) - sum(y0[[i]]$cum_diag_a[2,,2])
temp12 = sum(y0[[i]]$cum_diag_s[3,,2]) - sum(y0[[i]]$cum_diag_s[2,,2]) 

temp7 = sum(y0[[i]]$cum_diag_a[3,,1]) - sum(y0[[i]]$cum_diag_a[2,,1])
temp8 = sum(y0[[i]]$cum_diag_s[3,,1]) - sum(y0[[i]]$cum_diag_s[2,,1]) 

#temp1 = y0[[i]]$cum_diag_a[2,1,1] + y0[[i]]$cum_diag_a[2,2,1] + y0[[i]]$cum_diag_a[2,1,2] + y0[[i]]$cum_diag_a[2,2,2]
#temp2 = y0[[i]]$cum_diag_s[2,1,1] + y0[[i]]$cum_diag_s[2,2,1] + y0[[i]]$cum_diag_s[2,1,2] + y0[[i]]$cum_diag_s[2,2,2]

#temp3 = y0[[i]]$cum_diag_a[2,1,2] + y0[[i]]$cum_diag_a[2,2,2]
#temp4 = y0[[i]]$cum_diag_s[2,1,2] + y0[[i]]$cum_diag_s[2,2,2]


temp9 = sum(y0[[i]]$N[3,,2])
temp10 = sum(y0[[i]]$N[3,,3])


#print( (temp3+temp4)/(temp1+temp2))

tempN2a[i] =  (temp11+temp12)/(temp7+temp8)
tempN2b[i] = ( (temp3+temp4)/(temp6))
tempN2c[i] = ( (temp3+temp4)/(temp1+temp2))

tempN2d[i] = temp7+temp8

#tempN3d[i] = ( temp10/temp9)

}


p = plot(x = tempN2, y = tempN3, xlab = "Storing up to 1st diagnosis", ylab = "Storing up to 2nd diagnosis")
...
tempX = yPN[[i]]$phi[1,1,1] + yPN[[i]]$phi[1,2,1]
tempY = yPN[[i]]$notifiedandattended[1,1,1] + yPN[[i]]$notifiedandattended[1,2,1]
...
tempZ <- sum(yPN[[i]]$cum_diag_a[3,,]) +  sum(yPN[[i]]$cum_diag_s[3,,]) - (sum(yPN[[i]]$cum_diag_a[2,,]) +  sum(yPN[[i]]$cum_diag_s[2,,]))

gono_params[[i]]$kappa = 0.365*tempZ/tempY
}
