...
typedef struct old_model_internal {
  double *A0;
  double *beta_t;
  double *C;
  int dim_A;
...
old_model_internal* old_model_get_internal(SEXP internal_p, int closed_error);
static void old_model_finalise(SEXP internal_p);
SEXP old_model_create(SEXP user);
void old_model_initmod_desolve(void(* odeparms) (int *, double *));
SEXP old_model_contents(SEXP internal_p);
SEXP old_model_set_user(SEXP internal_p, SEXP user);
SEXP old_model_set_initial(SEXP internal_p, SEXP t_ptr, SEXP state_ptr, SEXP old_model_use_dde_ptr);
SEXP old_model_metadata(SEXP internal_p);
SEXP old_model_initial_conditions(SEXP internal_p, SEXP t_ptr);
void old_model_rhs(old_model_internal* internal, double t, double * state, double * dstatedt, double * output);
void old_model_rhs_dde(size_t neq, double t, double * state, double * dstatedt, void * internal);
void old_model_rhs_desolve(int * neq, double * t, double * state, double * dstatedt, double * output, int * np);
void old_model_output_dde(size_t n_eq, double t, double * state, size_t n_output, double * output, void * internal_p);
SEXP old_model_rhs_r(SEXP internal_p, SEXP t, SEXP state);
...
old_model_internal* old_model_get_internal(SEXP internal_p, int closed_error) {
  old_model_internal *internal = NULL;
  if (TYPEOF(internal_p) != EXTPTRSXP) {
    Rf_error("Expected an external pointer");
  }
  internal = (old_model_internal*) R_ExternalPtrAddr(internal_p);
  if (!internal && closed_error) {
    Rf_error("Pointer has been invalidated");
  }
  return internal;
}
void old_model_finalise(SEXP internal_p) {
  old_model_internal *internal = old_model_get_internal(internal_p, 0);
  if (internal_p) {
    cinterpolate_free(internal->interpolate_beta);
    cinterpolate_free(internal->interpolate_eta_h);
    cinterpolate_free(internal->interpolate_eta_l);
...
SEXP old_model_create(SEXP user) {
  old_model_internal *internal = (old_model_internal*) R_Calloc(1, old_model_internal);
  internal->A0 = NULL;
  internal->beta_t = NULL;
  internal->C = NULL;
  internal->entrants = NULL;
  ...
  internal->n_vax = 1;
  SEXP ptr = PROTECT(R_MakeExternalPtr(internal, R_NilValue, R_NilValue));
  R_RegisterCFinalizer(ptr, old_model_finalise);
  UNPROTECT(1);
  return ptr;
}
static old_model_internal *old_model_internal_ds;
void old_model_initmod_desolve(void(* odeparms) (int *, double *)) {
  static DL_FUNC get_desolve_gparms = NULL;
  if (get_desolve_gparms == NULL) {
    get_desolve_gparms =
  ...
  old_model_internal_ds = old_model_get_internal(get_desolve_gparms(), 1);
}
SEXP old_model_contents(SEXP internal_p) {
  old_model_internal *internal = old_model_get_internal(internal_p, 1);
...
void old_model_rhs_dde(size_t neq, double t, double * state, double * dstatedt, void * internal) {
  old_model_rhs((old_model_internal*)internal, t, state, dstatedt, NULL);
}
void old_model_rhs_desolve(int * neq, double * t, double * state, double * dstatedt, double * output, int * np) {
  old_model_rhs(old_model_internal_ds, *t, state, dstatedt, output);
}
void old_model_output_dde(size_t n_eq, double t, double * state, size_t n_output, double * output, void * internal_p) {
  old_model_internal *internal = (old_model_internal*) internal_p;
  double * U = state + 0;
  double * I = state + internal->dim_U;
  double * A = state + internal->offset_variable_A;
  double * S = state + internal->offset_variable_S;
...
SEXP old_model_rhs_r(SEXP internal_p, SEXP t, SEXP state) {
  ...
  UNPROTECT(1);
  double *output = REAL(output_ptr);
  old_model_rhs(internal, scalar_real(t, "t"), REAL(state), REAL(dstatedt), output);
  UNPROTECT(1);
  return dstatedt;
}
double user_get_scalar_double(SEXP user, const char *name,
