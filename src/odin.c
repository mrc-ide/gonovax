...
SEXP model_set_user(SEXP internal_p, SEXP user) {
  ...
  internal->dim_incidence_rate_1 = internal->n_group;
  ...
  if (output) {
    ...
    for (int i = 1; i <= internal->dim_incidence_rate_1; ++i) {
      for (int j = 1; j <= internal->dim_incidence_rate_2; ++j) {
        output[internal->offset_output_incidence_rate + i - 1 + internal->dim_incidence_rate_1 * (j - 1)] = internal->lambda[internal->dim_lambda_1 * (j - 1) + i - 1] * (1 - internal->vea[j - 1]);
      }
    }
