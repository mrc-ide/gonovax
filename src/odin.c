...
void model_rhs(model_internal* internal, double t, double * state, double * dstatedt, double * output) {
  ...
  for (int i = 1; i <= internal->dim_n_AU_1; ++i) {
    for (int j = 1; j <= internal->dim_n_AU_2; ++j) {
      internal->n_AU[i - 1 + internal->dim_n_AU_1 * (j - 1)] = (internal->mu == 0 || internal->nu == 0 ? (internal->nu / (double) (1 - internal->ved[j - 1]) * A[internal->dim_A_1 * (j - 1) + i - 1]) : 1 / (double) (internal->ved[j - 1] * ((1 / (double) internal->mu) - (1 / (double) internal->nu)) + (1 / (double) internal->nu)) * A[internal->dim_A_1 * (j - 1) + i - 1]);
    }
  }
...
void model_trial_stochastic_rhs(model_trial_stochastic_internal* internal, size_t step, double * state, double * state_next, double * output) {
  ...
  for (int i = 1; i <= internal->dim_r_AU_1; ++i) {
    for (int j = 1; j <= internal->dim_r_AU_2; ++j) {
      internal->r_AU[i - 1 + internal->dim_r_AU_1 * (j - 1)] = (internal->mu == 0 || internal->nu == 0 ? (internal->nu / (double) (1 - internal->ved[j - 1])) : 1 / (double) (internal->ved[j - 1] * ((1 / (double) internal->mu) - (1 / (double) internal->nu)) + (1 / (double) internal->nu)) * A[internal->dim_A_1 * (j - 1) + i - 1]);
    }
  }
  for (int i = 1; i <= internal->dim_screened_1; ++i) {
    for (int j = 1; j <= internal->dim_screened_2; ++j) {
