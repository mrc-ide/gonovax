...
SEXP model_trial_stochastic_set_user(SEXP internal_p, SEXP user) {
  ...
  for (int i = 1; i <= internal->dim_r_AU_1; ++i) {
    for (int j = 1; j <= internal->dim_r_AU_2; ++j) {
      internal->r_AU[i - 1 + internal->dim_r_AU_1 * (j - 1)] = (internal->mu == 0 || internal->nu == 0 ? (internal->nu / (double) (1 - internal->ved[j - 1])) : 1 / (double) (internal->ved[j - 1] * ((1 / (double) internal->mu) - (1 / (double) internal->nu)) + (1 / (double) internal->nu)));
    }
  }
