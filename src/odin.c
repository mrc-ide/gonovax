...
SEXP model_trial_stochastic_set_user(SEXP internal_p, SEXP user) {
  ...
  for (int i = 1; i <= internal->dim_r_AU_1; ++i) {
    for (int j = 1; j <= internal->dim_r_AU_2; ++j) {
      internal->r_AU[i - 1 + internal->dim_r_AU_1 * (j - 1)] = (internal->mu == 0 && internal->nu == 0 ? 0 : internal->nu * internal->mu / (double) (internal->ved[j - 1] * internal->nu + (1 - internal->ved[j - 1]) * internal->mu));
    }
  }
